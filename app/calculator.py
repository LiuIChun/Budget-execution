...
def summarize_execution(expense_df, budget_df):
    """Summarize committed and actual spending by department."""
    ...
    expense_summary = (
        expense_df.groupby("系所代碼", as_index=False)[
            ["動支金額", "實支金額"]
        ].sum()
    )
    ...
    summary["動支率(%)"] = summary.apply(
        lambda row: calculate_execution_rate(row["動支金額"], row["核定經費"]), axis=1
    )
    ...
    total_committed = summary["動支金額"].sum()
    total_committed_rate = calculate_execution_rate(total_committed, total_budget)
    total_actual = summary["實支金額"].sum()
    total_actual_rate = calculate_execution_rate(total_actual, total_budget)

    # Create total row with placeholder values for department_name and college
    total_row_data = {
        "系所代碼": "合計",
        "核定經費": total_budget,
        "動支金額": total_committed,
        "動支率(%)": total_committed_rate,
        "實支金額": total_actual,
